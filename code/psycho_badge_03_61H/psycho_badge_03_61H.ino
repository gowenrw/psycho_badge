...
// Preferences library for storing persistent settings
...
// Include CTF Library
// NO CTF YET... No Time, Sorry
// #include <psycho_badge_lib.h>

// Preferences object for storing persistent settings
...
unsigned char display_model = EPD_MODEL_61H;
...
// Display Image Names - must stay in the same order as DisplayImage[]
// Used for debugging to print the current display image name
const char* const DisplayImageNames[] = {
  "gImage_dying_reach_05",
  "gImage_psycho_hitchcock_02",
  "gImage_psycho_poster_ylw_01",
  "gImage_psycho_crazy_dress_07",
...
unsigned int ImgMax = 6;       // array index max number (0-based)
unsigned int ImgElement = 0;   // Should be set by preferences read in setup
unsigned int ImgElementPref;   // To be read from preferences in setup

int DisplayUpdateCooldownDefault = 1;  // loops to wait between display updates
...
void setup(){
  ...
  // Preferences Storage for reading/writing persistent data
  if (DebugSerial >= 2) {
    Serial.println("Open Preferences Namespace 'storage'");
  }
  // Open the "storage" namespace in read/write mode (false)
  // Namespaces group your keys and can be up to 15 characters long
  ...
  //
  // Read EINK Image Elements from storage if existing
  if (DebugSerial >= 2) {
    Serial.println("Get EINK ImgElement from Preferences");
  }
  ImgElementPref = preferences.getUInt("ImgElementPref", (ImgMax + 2));
  if (ImgElementPref > ImgMax) {
    ImgElementPref = 0;
    ImgElement = 0;
    preferences.putUInt("ImgElementPref", ImgElement);
    //
    // DISPLAY FIRST IMAGE IF NO PREFERENCES STORED
    if (DebugSerial >= 2) {
      Serial.println("Display EINK Initial Image");
    }
    fastDisplayImg();
  } else {
    ImgElement = ImgElementPref;
  }

  // Close Preferences Storage
  if (DebugSerial >= 2) {
    Serial.println("Close Preferences Namespace 'storage'");
  }
  preferences.end();
...
void loop(){
  ...
  if (DebugSerial >= 1) {
    ...
    Serial.println("  ********** ");
    Serial.println("  ** ");
    Serial.print("  DisplayImage="); Serial.print(DisplayImageNames[ImgElement]);
    Serial.println("  ** ");
    Serial.println("  ********** ");
    Serial.println("  ** ");
...
// //////////////////////////////////////////////////
void nextImgElement() {
    if (ImgElement == ImgMax) { ImgElement = 0; } else { ImgElement++; }
    // Preferences Storage for ImgElementPref
    preferences.begin("storage", false);
    preferences.putUInt("ImgElementPref", ImgElement);
    preferences.end();
}
//
void prevImgElement() {
    if (ImgElement == 0) { ImgElement = ImgMax; } else { ImgElement--; }
    // Preferences Storage for ImgElementPref
    preferences.begin("storage", false);
    preferences.putUInt("ImgElementPref", ImgElement);
    preferences.end();
